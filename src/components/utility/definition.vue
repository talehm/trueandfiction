...
<v-card class="mx-auto">
    <v-card-text class="pb-0">
        <span class="green--text font-weight-medium text--darken-2">{{ item.partOfSpeech }}</span> <span class="grey lighten-4 pa-1">{{index}} of {{ length }}</span>
    ...
    <v-card-actions>
        <v-btn-toggle vertical :model="expandedBtn" mandatory active-class="green white--text">
            <v-btn v-for="(btn,i) in btns" :key="i" small @click="showDetails(i)"> <span class="text-caption text-lowercase"> {{btn}} </span> </v-btn>
        </v-btn-toggle>
    </v-card-actions>
    <v-expand-transition>
        <div>
